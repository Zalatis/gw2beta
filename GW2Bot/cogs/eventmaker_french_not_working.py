...
class Eventmaker():
    ...
    async def event_add(self, ctx):
        ...
        bot_msg = await self.bot.say(
            "Entrez l'heure et la date (Format: HH:MM tz JJ/MM) ")
        ...
        for event in self.events[server.id]:
            if not event["has_started"]:
                et_str = dt.fromtimestamp(
                    event["create_time"], eastern).strftime("%H:%M %Z")
                ct_str = dt.fromtimestamp(
                    event["create_time"], central).strftime("%H:%M %Z")
                pt_str = dt.fromtimestamp(
                    event["create_time"], pacific).strftime("%d/%m %H:%M %Z")
                ...
                #         event["create_time"], central).strftime("%d/%m/%Y %H:%M"))
                emb.set_footer(
                ...
                #         event["event_start_time"], central).strftime("%d/%m/%Y %H:%M"))
                events.append(emb)
        if len(events) == 0:
                ...
                for event in self.events[server]:
                    if cur_time >= event["event_start_time"]\
                        ...
                        emb.add_field(
                            name="Activité: ", value=event["activity"])
                        pt_str = dt.fromtimestamp(
                            event["create_time"], pacific).strftime("%H:%M %d/%m %Z")
                        emb.add_field(
