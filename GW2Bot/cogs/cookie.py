...
class Cookie:
    ...
    async def _stealcd_heist(self, ctx, cooldown: int):
        """Définir le temps de recharge pour avoir volé des cookies"""
    ...
    async def _cookiecd_heist(self, ctx, cooldown: int):
        """Définir le temps de recharge pour l'obtention des cookies"""
        server = ctx.message.server
        settings = self.check_server_settings(server)
        if cooldown >= 0:
    ...
    async def give(self, ctx, user: discord.Member, cookies: int):
        """Donner des cookies à quelqu'un"""
        author = ctx.message.author
        settings = self.check_server_settings(author.server)
        if user.bot:
            return await self.bot.say("Bien essayé, nous les robots ne pouvons pas accepter les cookies des étrangers.")
        if author.id == user.id:
            return await self.bot.say("Vous ne pouvez pas vous donner de cookies.")
        ...
        if 0 < cookies <= sender_cookies:
            ...
            dataIO.save_json(self.file_path, self.system)
            msg = "Vous avez donné **{}** cookies à {}".format(cookies, user.name)
        else:
            msg = "Vous n'avez pas assez de cookies dans votre pot"
    ...
    async def jar(self, ctx):
        """Voyez combien de cookies sont dans votre pot."""
        author = ctx.message.author
        server = ctx.message.server
        settings = self.check_server_settings(server)
        self.account_check(settings, author)
    ...
    async def steal(self, ctx, user: discord.Member=None):
        ...
        if user == "Fail":
            ...
            return await self.bot.say("Le vol a échoué parce que la cible choisie est un bot.\nVous "
                                      "pouvez réessayer de voler à nouveau, votre temps de recharge n'est pas consommé.")
    ...
    def steal_logic(self, settings, user, author):
        ...
        if settings["Players"][user.id]["Cookies"] == 0:
            msg = ("ω(=｀ｪ ´=)ω Nyaa! Neko-chan est désolé, rien que des miettes dans le "
                   "pot de cette personne :cookie: !")
        else:
