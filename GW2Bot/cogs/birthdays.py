...
class Birthdays:
    """Annonce les anniversaires des personnes et leur donne un rôle d'anniversaire pour toute la journée"""
    ...
    # Message constants
    ROLE_SET = ":white_check_mark: Le rôle d'anniversaire pour **{s}** a été défini sur: **{r}**."
    BDAY_INVALID = ":x: La date d'anniversaire que vous avez entrée n'est pas valide. Elle doit être du format `JJ-MM`."
    BDAY_SET = ":white_check_mark: Votre anniversaire a été réglé sur: **{}**."
    CHANNEL_SET = ":white_check_mark: Le canal pour annoncer les anniversaires pour **{s}** a été défini sur: **{c}**."
    BDAY_REMOVED = ":put_litter_in_its_place: Votre anniversaire a été supprimé."
    ...
    async def bday_channel(self, ctx, channel: discord.Channel):
        """Définit le canal d'annonce d'anniversaire pour ce serveur"""
        message = ctx.message
        c = message.channel
    ...
    async def bday_role(self, ctx, role: discord.Role):
        """Définit le rôle d'anniversaire pour ce serveur"""
        message = ctx.message
    ...
    async def bday_remove(self, ctx):
        """Supprime votre date d'anniversaire"""
        message = ctx.message
        channel = message.channel
    ...
    async def bday_set(self, ctx, date, year: int=None):
        """Fixe votre date d'anniversaire

        La date doit être au format : JJ-MM
        L'année est facultative. Si non donné, l'âge ne sera pas affiché."""
        message = ctx.message
        channel = message.channel
        author = message.author
        ...
        if birthday is None:
            ...
            await self.bot.send_message(channel, self.BDAY_SET.format(bday_day_str + " " + bday_month_str))

    @bday.command(name="list", pass_context=True)
    async def bday_list(self, ctx):
        """Liste les anniversaires

        Si un utilisateur a son année définie, il affichera l'âge qu'il aura après son anniversaire cette année"""
        message = ctx.message
        channel = message.channel
        self.clean_bdays()
        self.save_data()
    ...
    async def handle_bday(self, user_id, year):
        ...
        if year is not None:
            ...
            embed.description = "<@!{}> a maintenant **{} ans**. :tada:".format(user_id, age)
        else:
            embed.description = "C'est l'anniversaire de <@!{}> aujourd'hui ! :tada:".format(user_id)
    ...
    def do_today_bdays(self):
        this_date = datetime.datetime.now().date().replace(year=1)
    ...
    def parse_date(self, date_str):
        result = None
        try:
            result = datetime.datetime.strptime(date_str, "%d-%m").date().replace(year=1)
        except ValueError:
            pass
        return result
