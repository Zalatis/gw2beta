...
class Mod:
    ...
    async def text(self, ctx, text: str, number: int):
        """Supprime les X derniers messages comportant le texte spécifié.

        Exemple:
        !cleanup text \"test\" 5

        N'oubliez pas d'utiliser les guillemets."""
        ...
        if not has_permissions:
            await self.bot.say("Je ne suis pas autorisé à supprimer des messages.")
            return
    ...
    async def user(self, ctx, user: discord.Member, number: int):
        """Supprime les X derniers messages de l'utilisateur spécifié.

        Exemples:
        !cleanup user @\u200bZalati 2
        !cleanup user Zalati 6"""
    ...
    async def after(self, ctx, message_id : int):
        """Supprime tout les messages après le message spécifié

        Pour obtenir un identifiant de message, activez le mode
        développeur de Discord paramètres, onglet 'apparence'.
        Puis faites un clic droit sur un message et copiez
        son identifiant.
        """
        ...
        if not is_bot:
            await self.bot.say("Cette commande peut être utilisé seulement par des bots "
                               "avec un compte bot.")
            return
        ...
        if not has_permissions:
            ...
            await self.bot.say("Message non trouvé.")
            return
    ...
    async def cleanup_bot(self, ctx, number: int):
        """Supprime les messages de commande et les messages du bot"""
        ...
        if isinstance(prefixes, str):
            ...
            if asyncio.iscoroutine(prefixes):
                await self.bot.say('Les préfixes Coroutine ne sont pas encore implémentés.')
                return
    ...
    async def reason(self, ctx, case, *, reason : str=""):
        ...
        try:
            ...
            await self.bot.say("Ce cas ne vous appartient pas.")
        except KeyError:
            await self.bot.say("Ce cas n'existe pas.")
        except NoModLogChannel:
            await self.bot.say("Il n'y a pas de jeu de canaux mod-log.")
        except CaseMessageNotFound:
            await self.bot.say("Je n'ai pas pu trouver le message du cas.")
        except NoModLogAccess:
            await self.bot.say("Je ne suis pas autorisé à accéder au mod-log "
                               "salon")
        else:
            await self.bot.say("Cas #{} mis à jour.".format(case))
