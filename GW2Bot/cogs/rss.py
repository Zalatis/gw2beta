...
class RSS(object):
    ...
    async def _rss_add(self, ctx, name: str, url: str):
        """Ajoute un flux rss au canal actuel"""
    ...
    async def _rss_template(self, ctx, feed_name: str, *, template: str):
        ("""Changer le template pour les flux

        Chaque variable doit commencer par $, variables valides
        ...
        if success:
            await self.bot.say("Template ajouté avec succès.")
        else:
            await self.bot.say('Flux non trouvé!')

    @rss.command(pass_context=True, name="force")
    async def _rss_force(self, ctx, feed_name: str):
        """Force l'alerte du rss"""
        server = ctx.message.server
        channel = ctx.message.channel
        feeds = self.feeds.get_copy()
        if server.id not in feeds:
            await self.bot.say("Il n'y a pas de flux pour ce serveur")
            return
        if channel.id not in feeds[server.id]:
            await self.bot.say("Il n'y a pas de flux pour ce canal.")
            return
        if feed_name not in feeds[server.id][channel.id]:
            await self.bot.say("Ce nom de flux n'existe pas.")
            return
    ...
    async def _rss_remove(self, ctx, name: str):
        """Retire un flux de ce server"""
        success = await self.feeds.delete_feed(ctx, name)
        if success:
            await self.bot.say('Flux supprimé.')
        else:
            await self.bot.say('Flux non trouvé!')
